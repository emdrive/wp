...
echo '<div class="wrap">';
	...
	";
	echo '
		<form method="post" name="payform" action="'.get_bloginfo('home').'/admin/admin.php?page='.$pageaction.'"">
			<div class="metabox-holder has-right-sidebar" id="poststuff">
				...
				<div id="post-body">
								...
								<div class="inside">
									<div id="acf-username" class="field field_type-text" >
										...
										<div class="acf-input-wrap">
											<input id="acf-field-username" class="text username" name="username" value="" placeholder="" type="text">
										</div>
									</div>
									<div id="acf-type" class="field field_type-radio ">
										...
										<ul class="acf-radio-list radio horizontal">
											<li>
												<label><input checked name="pay_type" value="xiaofei" type="radio">消费币</label>
												<label><input name="pay_type" value="pay_type jihuo" type="radio">激活币</label>
											</li>
										</ul>
									</div>
									<div id="acf-number" class="field field_type-text" >
										...
										<div class="acf-input-wrap">
											<input class="text pay_number" name="pay_number" value="" type="text">
										</div>
									</div>
									<div id="acf-note" class="field field_type-textarea">
										<p class="label"><label for="acf-field-note">备注</label></p>
										<textarea class="textarea pay_note" name="pay_note" rows="3"></textarea>
									</div>
								</div>
							</div>
						</div>
					...
					<div class="" id="side-info-column">
						<input name="submit" class="acf-button pay-submit" value="确定充值" type="submit">
					</div>
				</div>
			</div>
		</form>';
...
function action_pay_data_func(){
	...
	if ( username_exists($_REQUEST['username']) ){
		...
		if($pay_type == "xiaofei"){
			...
			$newtoxiaofeibi = ( $xiaofeibi+$_REQUEST['pay_number'] );
			update_user_meta( $uid,'xiaofeibi',$newtoxiaofeibi );
			$type = "消费币";
		}else{ 
			$jihuobi          = get_user_meta($uid,'jihuobi',true);
			$active_xiaofeibi = get_field('active_xiaofeibi','options');
			$newtojihuobi = ( $jihuobi+$_REQUEST['pay_number'] );
			if( $newtojihuobi >= $active_xiaofeibi && $user_status == 0 ){
				$wpdb->query("UPDATE $table_name SET user_status = 1 WHERE ID =".$uid);
			}
			update_user_meta( $uid,'jihuobi',$newtojihuobi );
			$type = "激活币";
		}
		$pay_array = array(
			'post_title'     => $_REQUEST['username'],
			'post_status'    => 'publish',
			'post_type'      => 'type_pay_record',
			'post_content'   => "给用户".$_REQUEST['username']." 充值".$type."：".$_REQUEST['pay_number']." 个",
			'post_author'    => $current_userID
		);
		$pid = wp_insert_post( $pay_array );
		if($pid){
			add_post_meta( $pid,'numbers',$_REQUEST['pay_number'],true ) or update_post_meta( $pid,'numbers',$_REQUEST['pay_number'] );
			add_post_meta( $pid,'price_type',$type,true ) or update_post_meta( $pid,'price_type',$type );
		}
		echo '1';
	}else{
		echo $status = '该用户不存在';
...
add_action("wp_ajax_action_pay_data_func", "action_pay_data_func");
...
//提现审核
function func_tx_reason_action(){
	...
	if(!empty($pid)){
		...
		$uid       = $post_info->post_author;
		$benjin_price  = get_user_meta($uid,'benjin_price',true);
		$lixi_price    = get_user_meta($uid,'lixi_price',true);
		$jifen_price   = get_user_meta($uid,'jifen_price',true);

		$tixian_type    = get_post_meta( $pid,'tixian_type',true );
		$fact_price     = get_post_meta( $pid,'fact_price',true );
		$tixian_jifen   = get_post_meta( $pid,'tixian_jifen',true );
		$new_price      = get_post_meta( $pid,'new_price',true );
		$tixian_status  = get_post_meta( $pid,'tixian_status',true );
		$tixian_replay  = get_post_meta( $pid,'tixian_replay',true );

		if($sid == 0){

			update_post_meta( $pid,'tixian_status',1 );
			add_post_meta( $pid,'pay_apply_time',$datetime,true ) or update_post_meta( $pid,'pay_apply_time',$datetime );
			$current_p = $benjin_price - $fact_price;
			$current_l = $lixi_price - $fact_price;
			$current_j = $jifen_price - $tixian_jifen;
			echo 1;

		}else{

			update_post_meta( $pid,'tixian_status',0 );
			add_post_meta( $pid,'pay_apply_time',$datetime,true ) or update_post_meta( $pid,'pay_apply_time',$datetime );
			$current_p = $benjin_price + $fact_price;
			$current_l = $lixi_price + $fact_price;
			$current_j = $jifen_price + $tixian_jifen;
		...
		if($tixian_type == 'benjin_price'){
			update_user_meta($uid,'level_1',0);
			update_user_meta($uid,'level_2',0);
			update_user_meta($uid,'level_3',0);
			update_user_meta($uid,'level_4',0);
			update_user_meta($uid,'benjin_price',$current_p);
		}else{
			update_user_meta($uid,'lixi_price',$current_l);
			update_user_meta($uid,'jifen_price',$current_j);
		}
	}
	die();
}
add_action("wp_ajax_func_tx_reason_action", "func_tx_reason_action");
