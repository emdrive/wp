...
function my_manage_type_tixian_columns( $column, $post_id ) {
	...
	switch( $column ) {
		...
		case 'action' :
			...
			echo '<button rel="'.$post_id.'" data="'.$tixian_status.'" class="reason-btn btn btn'.$post_id.' green">'.$text.'</button>';
			break;	
		default :
			break;
	}
	...
	if( $second_password == $secpsw ){
		...
		if($tixian_type == "lixi"){
			...
			if( $tixian_price > $lixi_price ){
				echo '提现金额超出，您只有利息 ( ￥'.$lixi_price.' )！'; exit;
			}else{ 
				$new_price  = ($lixi_price-$tixian_price); 
				$new_txp_jf = ($lixi_tixian[0]['jifen_package']*$tixian_price*0.01);
				$new_txp    = ($lixi_tixian[0]['cash']*$tixian_price*0.01);
				$new_price  = $lixi_price-$tixian_price;
				$new_key    = 'lixi_price'; 
			}
		}else{
			...
			if( $tixian_price > $benjin_price ){
				echo '提现金额超出，您只有本金 ( ￥'.$benjin_price.' )！'; exit;
			}else{ 
				$new_key    = 'benjin_price';
				$new_txp_jf = '0';
				$new_txp    = ($benjin_tixian_bili*$tixian_price*0.01);
				$new_price  = $benjin_price-$tixian_price;
			}
		}
...
function get_tuijian_calculate($level,$field){
	...
	if( $get_count_level < $level[0][$field.'_p_num'] ){
		$note = "未达到".$level[0][$field.'_p_num']."人以上奖金( ".$level[0][$field.'_jiangjin']."% )不可提现";
	}else{
		$note = "奖金( ".$level[0][$field.'_jiangjin']."% )可以随时提现,已存入本金钱包";
	}
	...
	if($level_id){
		update_user_meta($uid,'benjin_price',($price+$benjin_price));
		if($field == "one_level"){
	...
	if( $c_oneIDs >= $zhitui_team_num && $total_teams >= $get_team_num ){
		...
		if($field == "gold_planner"){
	        ...
	        if(count($silver_count) >= 2){
	        	...
	        	if($topid){
					update_user_meta($uid,'benjin_price',($jiangli+$benjin_price));
					$note = "升级为金牌理财师";
